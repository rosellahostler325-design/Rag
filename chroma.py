...
# 注意：需要安装对应的 embedding 库
# 如果使用 OpenAI: pip install langchain-openai
# 如果使用百度千帆: pip install langchain-community
# 如果使用本地模型: pip install sentence-transformers
# 这里使用 sentence-transformers 作为示例（本地模型，无需 API key）
try:
...
class Config:
    ...
    persist_directory: str = "/home/user/文档/RL4LLM/Rag_data/vector_db/chroma"
    ...
    # 可选模型（按推荐程度排序）:
    # "shibing624/text2vec-base-chinese"  # 专为中文优化，约400MB
    # "BAAI/bge-m3"  # 最佳多语言模型，约2.3GB
    # "sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2"  # 默认，较小
...
def create_embedding_model(config: Config):
    ...
    if "multilingual-MiniLM" in config.embedding_model:
        print("提示: 如需更好的中文检索效果，建议使用 'shibing624/text2vec-base-chinese' 或 'BAAI/bge-m3'")
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='构建和检索 Chroma 向量数据库')
    ...
    parser.add_argument('--model', type=str, help='Embedding 模型名称')
    ...
    try:
        ...
        print("="*50)
        print("\n提示:")
        print("1. 如果检索效果不理想，建议使用更好的中文Embedding模型")
        print("   推荐: 'shibing624/text2vec-base-chinese' 或 'BAAI/bge-m3'")
        print("2. 可以调整 chunk_size 和 chunk_overlap 参数优化分割效果")
        print("3. MMR 的 lambda_mult 参数可以调整多样性和相关性的平衡")
        print("4. 使用 --interactive 参数进入交互式搜索模式")
        print("5. 使用 --rebuild 参数强制重建向量数据库")
